...
# Pour chaque bougie, vérifier si dans les 12 bougies suivantes (48h),
# le High dépasse Close * POURCENT_MARGE
"""
window = 48  # 12 bougies (48h en 4h)
drop_window = 48  # période où on regarde la baisse (lendemain + surlendemain)
DROP_THRESHOLD = 0.99  # -1%

future_high = btc_daily["High"].rolling(window=window, min_periods=1).max().shift(-window)
future_low = btc_daily["Low"].rolling(window=drop_window, min_periods=1).min().shift(-window)

btc_daily["Target"] = (
    (future_high > btc_daily["Close"] * POURCENT_MARGE)  # hausse détectée
    & (future_low >= btc_daily["Close"] * DROP_THRESHOLD)  # pas de baisse de -1%
).astype(int)"""

"""window = 12  # 48h en bougies 4H
btc_daily["Target"] = (
    btc_daily["High"].rolling(window=window, min_periods=1).max().shift(-window)
    > btc_daily["Close"] * POURCENT_MARGE
).astype(int)
"""
POURCENT_HAUSSE = 0.0075   # +0.75%
POURCENT_BAISSE = 0.01     # -1%
window = 48  # ex: 12 bougies 4H ≈ 48h

# 3. Définir la Target en se basant sur les bougies horaires
targets = []
for day in btc_daily.index:
    if day + pd.Timedelta(days=2) > btc_hourly.index[-1]:
        targets.append(None)  # pas assez de données futures
        continue
    
    close_price = btc_daily.loc[day, "Close"]
    future_window = btc_hourly.loc[day + pd.Timedelta(days=1): day + pd.Timedelta(days=2)]

    target = 0
    seen_hausse = False
    for _, row in future_window.iterrows():
        if not seen_hausse and row["High"] >= close_price * (1 + POURCENT_HAUSSE):
            seen_hausse = True
            target = 1
        if row["Low"] <= close_price * (1 - POURCENT_BAISSE):
            if not seen_hausse:
                target = 0
            break
